<title>{{ $headTitle }} - Cờ Tướng - Chơi cờ tướng với máy - Choi co tuong voi may</title>
...
<meta name="description" content="Cùng chơi Cờ tướng với nhiều tính năng hấp dẫn như Cờ thế, Tạo phòng, Chơi với nhau, Chơi với máy (4 cấp độ: Mới chơi, Dễ, Bình thường và Khó) và nhất là không cần đăng ký! {{ $headTitle }}" />
<meta property="og:title" content="{{ $headTitle }} - Cờ Tướng - Chơi cờ tướng với máy - Choi co tuong voi may" />
...
<style amp-custom>
@php
    $fa = file_get_contents(URL::to('/').'/css/fa/css/all-amp.css');
    $bootstrap = file_get_contents(URL::to('/').'/css/bootstrap-amp.css');
    $lumen = file_get_contents('https://stackpath.bootstrapcdn.com/bootswatch/4.5.0/lumen/bootstrap.min.css');
    $datatables = file_get_contents('https://cdn.datatables.net/1.10.21/css/dataTables.bootstrap4.min.css');
    $main = file_get_contents(URL::to('/').'/css/index.css');
    echo str_replace('!important', '', $fa.$bootstrap.$lumen.$datatables.$main);
@endphp
</style>
...
{
  "vars" : {
    "gtag_id": "UA-158792011-1",
    "config" : {
      "UA-158792011-1": { "groups": "default" }
    }
  }
}
</script>
</amp-analytics>
<amp-auto-ads type="adsense" data-ad-client="ca-pub-3585118770961536">
</amp-auto-ads>