  ...
  <body class="{{ $bodyClass }}">
    ...
    <main>
      <div class="container-fluid game px-0" itemscope itemtype="http://schema.org/Game">
        <div class="container {{ url()->current() == URL::to('/set-up/') ? 'px-3 pb-0 pt-3' : 'p-3' }}">
          <audio id="nuoc-co">
          ...
          @if ( url()->current() == URL::to('/set-up/') )
          <p class="w-100 text-center">
            <a id="capture" class="btn btn-danger btn-lg" href="javascript:void(0);"><i class="fal fa-camera"></i> Capture the board</a>
          </p>
          @endif
          ...
          <p class="w-100 text-center my-4">
            <a id="tao-phong" data-phong="<?php echo md5(time()); ?>" data-url="{{ URL::to('/') }}/room/<?php echo md5(time()); ?>" class="btn btn-success btn-lg{{ $roomCode == '' ? ' pulse': '' }}"><i class="fad fa-plus-circle"></i> Host a room</a>
          </p>
          @yield('aboveContent')
          <div class="row">
            ...
            @if (url()->current() != URL::to('/set-up/'))
            <p class="w-100 text-center mt-2">
              <a id="share-board" class="mx-auto btn btn-success btn-lg pulse py-2"><i class="fad fa-share"></i> Share board</a>
            </p>
            <script>
            $('#share-board').on('click', function(){
              window.location.href = "{{ URL::to('/share/') }}/" + game.fen();
            });
            </script>
            @endif
          </div>
