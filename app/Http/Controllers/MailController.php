...
class MailController extends Controller
{
    ...
    public function gui(Request $request)
    {
        ...
        $subject = $request->input('subject');
        if (!$request->input('name') || $name === '') {
            echo json_encode(array('message' => 'Họ tên không được để trống', 'code' => 0));
            exit();
        }
        if (!$request->input('email') || $email === '') {
            echo json_encode(array('message' => 'Email không được để trống', 'code' => 0));
            exit();
        } else if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            echo json_encode(array('message' => 'Định dạng email sai', 'code' => 0));
            exit();
        }
        if (!$request->input('subject') || $subject === '') {
            echo json_encode(array('message' => 'Chủ đề không được để trống', 'code' => 0));
            exit();
        }
        if (!$request->input('message') || $message === '') {
            echo json_encode(array('message' => 'Tin nhắn không được để trống', 'code' => 0));
            exit();
        }
